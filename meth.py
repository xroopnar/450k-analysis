...
def snp_filter(data):
    probes = [ x for x in self.data.columns if x[0:1]=="c"]
    data = data[probes]

def batch_filter(meta,data,cutoff=50):
    #cutoff = 50
    counts = meta.ExperimentID.value_counts()
    counts = counts[counts>cutoff]
    #print(counts.shape)
    meta = meta[meta.ExperimentID.isin(counts.index)]
    hits = data.loc[meta.index]
    print(hits.shape)
    counts = meta.TissueName.value_counts()
    counts = counts[counts>cutoff]
    #print(counts.shape)
    meta = meta[meta.TissueName.isin(counts.index)]
    #print(meta.shape)
    hits = hits.loc[meta.index]
    #print(hits.shape)
    return(hits)
